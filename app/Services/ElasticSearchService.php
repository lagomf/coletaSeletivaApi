...
{
    private $elastic_url, $elastic_key;

    public function __construct()
    {
        $this->elastic_url = env("ELASTICSEARCH_API_URL");
        $this->elastic_key = env("ELASTICSEARCH_KEY");
    }
    /**
     * Makes API call to an endpoint at ElasticSearch.
     *
    ...
    private function request($endpoint, $method = 'GET', $body = [], $headers = [], $params = [], $http_errors = true) {
        $exact_headers = [
            'Authorization' => $this->elastic_key,
            'Accept-Encoding' => 'gzip, deflate, br'
        ];

        $headers = array_merge($exact_headers, $headers ?? []);

        $elastic_api_url = $this->elastic_url;
        $url = "$elastic_api_url/$endpoint";
